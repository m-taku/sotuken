...
namespace smEngine {
	...
	public:
		...
		*/
		...
		void NewExecution()
		{
			...
			{
				m_gameObjectListArray.at(Newobject->m_priority).push_back(Newobject);
			}
			m_newList.clear();
		}
		template<class T>
	...
	private:
		...
		std::vector<IGameObject*> m_newList;								//newの際の保存場所（newGOの順序を無視して同時にするため）
